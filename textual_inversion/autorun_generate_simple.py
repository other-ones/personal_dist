...
for gen_idx,gen_target_step in enumerate(gen_target_step_list):
    for cidx,concept in enumerate(concepts):
        ...
        for exp_idx,exp in enumerate(exps):
            if not ('mlm000001' in exp or 'mlm00001' in exp or'nomlm' in exp):
                continue
            ...
            output_dir=os.path.join('results_simple/ti_results/{}/{}'.format(dir_name,concept))
            ...
            while True:
                ...
                time.sleep(delay)
            print(f'{exp_name}\tDEVICE: {device_idx}\t{cidx+1}/{len(concepts)}\t{exp_idx+1}/{len(exps)}')
