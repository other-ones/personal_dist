...
for gen_idx,gen_target_step in enumerate(gen_target_step_list):
    for cidx,concept in enumerate(concepts):
        ...
        for exp_idx,exp in enumerate(exps):
            if not 'nomlm' in exp:
                continue
            for lower in [0.1,0.15,0.2,0.25]:
                lower_str=str(lower*10).replace('.','')
                if '_rev' in exp:
                    rev=1
                else:
                    rev=0
                ...
                exp_name+='_lower{}'.format(lower_str)
                output_dir=os.path.join('results_lower/ti_results/{}/{}'.format(dir_name,concept))
                ...
                while True:
                    ...
                    if found:
                        break
                    print(f'SLEEP GENEARTING\t{exp_name}\t{cidx+1}/{len(concepts)}\t{gen_idx+1}/{len(gen_target_step_list)}')
                    time.sleep(delay)
